...
# Create a frame that will be scrolled
scrollable_frame = tk.Frame(canvas)

# Add the scrollable frame to a window in the canvas
canvas.create_window((0, 0), window=scrollable_frame, anchor="nw")

# Update the scrollregion whenever the size of the scrollable_frame changes
def on_frame_configure(event):
    canvas.configure(scrollregion=canvas.bbox("all"))
...
# Add a button to start comparing images
compare_button = tk.Button(scrollable_frame, text="Start Comparing", command=compare_images)
compare_button.grid(row=2, column=0, columnspan=4, pady=10)

# Add a label to display the results
result_text = tk.StringVar()
result_label = tk.Label(scrollable_frame, textvariable=result_text, font=("Arial", 14))
result_label.grid(row=3, column=0, columnspan=4, pady=20)
